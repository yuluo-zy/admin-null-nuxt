<template>
  <!-- Section 1 -->
  <section
    class="box-border w-full bg-gray-100 relative block py-24 overflow-hidden leading-6 text-left bg-white"
  >
    <div class="max-w-6xl px-4 mx-auto leading-6 text-left xl:px-12">
      <div
        class="box-border flex flex-col flex-wrap items-start mx-0 text-indigo-900"
      >
        <div class="pb-4 text-sm font-bold text-left text-gray-700 uppercase">
          Powerful Features
        </div>
        <h3
          class="relative w-full px-0 pb-2 m-0 font-sans text-4xl font-bold text-left xl:flex-grow-0 xl:flex-shrink-0 md:flex-grow-0 md:flex-shrink-0 lg:flex-grow-0 lg:flex-shrink-0"
        >
          Powerful and Simple Features for your Application
        </h3>
      </div>
      <div class="grid grid-cols-3 gap-5 text-indigo-900 md:grid-cols-6">
        <div class="relative w-full col-span-3 px-4 leading-6 text-left">
          <div
            class="box-border flex items-center justify-start pt-12 text-indigo-900"
          >
            <img
              src="https://cdn.devdojo.com/tails/icons/telephone.png"
              class="leading-6 text-left align-middle border-none w-14 h-14"
            />
            <div class="pl-8 leading-6 text-left">
              <div class="box-border pb-1 text-xl font-medium text-indigo-900">
                100% Responsive
              </div>
              <div class="box-border text-base leading-normal text-gray-700">
                Responsive and mobile ready templates
              </div>
            </div>
          </div>
        </div>

        <div class="relative w-full col-span-3 px-4 leading-6 text-left">
          <div
            class="box-border flex items-center justify-start pt-12 text-indigo-900"
          >
            <img
              src="https://cdn.devdojo.com/tails/icons/tips.png"
              class="leading-6 text-left align-middle border-none w-14 h-14"
            />
            <div class="pl-8 leading-6 text-left">
              <div class="box-border pb-1 text-xl font-medium text-indigo-900">
                Insights &amp; Tips
              </div>
              <div class="box-border text-base leading-normal text-gray-700">
                Insights and tips for productivity
              </div>
            </div>
          </div>
        </div>

        <div class="relative w-full col-span-3 px-4 leading-6 text-left">
          <div
            class="box-border flex items-center justify-start pt-12 text-indigo-900"
          >
            <img
              src="https://cdn.devdojo.com/tails/icons/mail-inbox-app.png"
              class="leading-6 text-left align-middle border-none w-14 h-14"
            />
            <div class="pl-8 leading-6 text-left">
              <div class="box-border pb-1 text-xl font-medium text-indigo-900">
                Automated Emails
              </div>
              <div class="box-border text-base leading-normal text-gray-700">
                Easily send out automated emails
              </div>
            </div>
          </div>
        </div>

        <div class="relative w-full col-span-3 px-4 leading-6 text-left">
          <div
            class="box-border flex items-center justify-start pt-12 text-indigo-900"
          >
            <img
              src="https://cdn.devdojo.com/tails/icons/clock.png"
              class="leading-6 text-left align-middle border-none w-14 h-14"
            />
            <div class="pl-8 leading-6 text-left">
              <div class="box-border pb-1 text-xl font-medium text-indigo-900">
                Queued Jobs
              </div>
              <div class="box-border text-base leading-normal text-gray-700">
                Set it and forget it with jobs
              </div>
            </div>
          </div>
        </div>

        <div class="relative w-full col-span-3 px-4 leading-6 text-left">
          <div
            class="box-border flex items-center justify-start pt-12 text-indigo-900"
          >
            <img
              src="https://cdn.devdojo.com/tails/icons/calendar.png"
              class="leading-6 text-left align-middle border-none w-14 h-14"
            />
            <div class="pl-8 leading-6 text-left">
              <div class="box-border pb-1 text-xl font-medium text-indigo-900">
                Schedulers
              </div>
              <div class="box-border text-base leading-normal text-gray-700">
                Schedule your automations and jobs
              </div>
            </div>
          </div>
        </div>

        <div class="relative w-full col-span-3 px-4 leading-6 text-left">
          <div
            class="box-border flex items-center justify-start pt-12 text-indigo-900"
          >
            <img
              src="https://cdn.devdojo.com/tails/icons/bank.png"
              class="leading-6 text-left align-middle border-none w-14 h-14"
            />
            <div class="pl-8 leading-6 text-left">
              <div class="box-border pb-1 text-xl font-medium text-indigo-900">
                Simple Billing
              </div>
              <div class="box-border text-base leading-normal text-gray-700">
                Simple and easy billing system
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script>
export default {
  name: 'LongMaoFeatures',
}
</script>

<style scoped></style>
